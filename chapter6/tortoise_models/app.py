...
from chapter6.tortoise_models.models import (
    PostDB,
    PostCreate,
    PostPartialUpdate,
    PostTortoise,
...
async def list_posts(pagination: Tuple[int, int] = Depends(pagination)) -> List[PostDB]:
    ...
    results = [PostDB.model_config(post) for post in posts]
...
async def get_post(post: PostTortoise = Depends(get_post_or_404)) -> PostDB:
    return PostDB.model_config(post)
...
async def create_post(post: PostCreate) -> PostDB:
    ...
    return PostDB.model_config(post_tortoise)
...
TORTOISE_ORM = {
    "connections": {"default": "sqlite://chapter6_tortoise_models.db"},
    "apps": {
        "models": {
            "models": ["chapter6.tortoise_models.models"],
